...
F65 在实际模型/CLI 调用入口保存不可变元数据，F66 再提供浏览器观察入口。当前 GET 接口只供已认证 Owner，不会启动执行、重新生成输入、读取新的私有记忆或改变 Agent 权限。
...
新增一张 `context_receipts` 表，update/delete/replace均拒绝。旧记录不补造摘要；离线备份与恢复包含该表。准备完成后在外部调用前崩溃仍可能留下prepared而没有真正执行，这是保留该阶段名称的原因。工具内部命令事件及浏览器摘要入口另行实现，不把本回执当作完整工具日志。
