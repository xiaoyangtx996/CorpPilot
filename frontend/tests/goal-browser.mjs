...
try {
  ...
  const activityCallsBefore = events().filter(e => ['model_call', 'cli_call', 'goal_post'].includes(e.kind)).length;
  await page.getByLabel('查看会话成员', { exact: true }).selectOption(manifest.agent_ids[1]);
  let activityPanel = page.getByRole('region', { name: `${manifest.agent_names[1]}的活动`, exact: true });
  await activityPanel.getByRole('button', { name: '刷新 Agent 活动', exact: true }).click();
  await activityPanel.getByText(`执行 ${finalId} · 第1次 · 当次需求 v1`, { exact: true }).waitFor();
  assert.equal(await activityPanel.getByText('当次 Owner 验收：已批准', { exact: true }).count(), 1);
  const activityReply = await context.request.get(`${baseURL}/api/workbench/agents/${manifest.agent_ids[1]}/activity`, { headers: { Authorization: `Bearer ${manifest.access_token}` } });
  assert.equal(activityReply.status(), 200);
  const observed = await activityReply.json();
  ...
  await page.getByRole('dialog').getByText('成果与 Owner 评审', { exact: true }).waitFor();
  await page.getByRole('dialog').getByText(usageText, { exact: true }).waitFor();
  await page.screenshot({ path: path.join(outDir, 'cli-usage.png') });
  await page.getByRole('dialog').getByRole('button', { name: '关闭', exact: true }).click();
  const usagePath = `**/api/workbench/agents/${manifest.agent_ids[1]}/activity`;
  const missingUsage = structuredClone(observed);
  missingUsage.executions.items[0].usage = null;
  await page.route(usagePath, route => route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify(missingUsage) }));
  await activityPanel.getByRole('button', { name: '刷新 Agent 活动', exact: true }).click();
  await activityPanel.getByText('CLI 用量未取得有效回执，不能视为零费用。', { exact: true }).waitFor();
  assert.equal(await activityPanel.getByText(usageText, { exact: true }).count(), 1);
  await page.unroute(usagePath);
  await activityPanel.getByRole('button', { name: '刷新 Agent 活动', exact: true }).click();
  await activityPanel.getByText(usageText, { exact: true }).nth(1).waitFor();
  await page.getByLabel('查看会话成员', { exact: true }).selectOption(manifest.coordinator_id);
  activityPanel = page.getByRole('region', { name: `${manifest.agent_names[0]}的活动`, exact: true });
  ...
  await activityPanel.getByRole('button', { name: '刷新 Agent 活动', exact: true }).click();
  await activityPanel.getByRole('alert').filter({ hasText: '未确认活动为空' }).waitFor();
  assert.equal(await activityPanel.getByText('尚无执行记录。', { exact: true }).count(), 0);
  await page.unroute(activityPath);
  await activityPanel.getByRole('button', { name: '刷新 Agent 活动', exact: true }).click();
  await activityPanel.getByText('尚无执行记录。', { exact: true }).waitFor();
  assert.equal(events().filter(e => ['model_call', 'cli_call', 'goal_post'].includes(e.kind)).length, activityCallsBefore);
  await page.screenshot({ path: path.join(outDir, 'agent-activity.png') });
  report.agentActivity = { passed: true, executionAgentId: manifest.agent_ids[1], tasks: observed.tasks.total, executions: observed.executions.total, modelAgentId: manifest.coordinator_id, noNewCalls: true };
