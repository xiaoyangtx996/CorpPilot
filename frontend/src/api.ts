...
export type ModelSettingsValue = { enabled: boolean; transport?: 'http' | 'opencode'; executable?: string; model: string; base_url: string; api_key_env: string; max_output_tokens: number | null; timeout_seconds: number | null; rpm: number | null; max_concurrency: number | null; configured: boolean; credential_available: boolean; executable_available?: boolean; platform_supported?: boolean };
export type CliSettingsValue = { resource_admission_enabled: boolean; host_reserve_memory_mb: number; local_worker_memory_mb: number; local_worker_cpus: number; enabled: boolean; engine?: 'codex' | 'opencode'; backend: 'local' | 'docker'; executable: string; docker_executable: string; docker_image: string; docker_cpus: number; docker_memory_mb: number; docker_pids_limit: number; model: string; api_key_env: string; timeout_seconds: number; max_concurrency: number; configured: boolean; credential_available: boolean; executable_available: boolean; platform_supported: boolean };
...
export type ReplyRun = { attempt: number; requirement_version: number; id: string; conversation_id: string; agent_id: string; source_message_id: string; request_id: string; state: 'queued' | 'running' | 'completed' | 'failed' | 'cancelled' | 'unknown'; model: string | null; usage: { prompt_tokens: number | null; completion_tokens: number | null } | null; error: string | null; created_at: string; updated_at: string; reply_message_id: string | null };
export type ModelUnknownRun = ReplyRun & { attempt: number; requirement_version: number; kind?: 'reply' | 'planning' | 'retrospective' | 'peer_review'; scope?: 'agent' | 'project'; scope_id?: string };
...
export type TaskExecution = { usage: { input_tokens: number | null; output_tokens: number | null; cached_input_tokens: number | null } | null; id: string; task_id: string; agent_id: string; requirement_version: number; attempt: number; request_id: string; reconciliation_note: string; previous_execution_id: string | null; state: 'queued' | 'running' | 'stopping' | 'awaiting_review' | 'failed' | 'cancelled' | 'unknown' | 'superseded'; exit_code: number | null; summary: string | null; created_at: string; updated_at: string };
...
export type GoalExecutionRequest = { request_id: string; source_message_id: string; agent_id: string; candidate_ids: string[]; shared_brief: string; max_tasks: number; confirm_execution: true; confirm_handoff: boolean };
...
export type BudgetSettings = BudgetConfig & { currency: 'USD'; revision: number; reserved_micro_usd: number; unsettled_reserved_micro_usd: number; settled_micro_usd: number; committed_micro_usd: number; settlement_count: number; available_micro_usd: number; reservation_count: number };
...
export type FeeRequest = { request_id: string; attempt: number; requirement_version: number; expected_revision: number; amount_micro_usd: number; evidence_reference: string; note: string; confirm: true };
export type FeeReceipt = { kind: 'model' | 'cli'; run_id: string; agent_id: string; attempt: number; requirement_version: number; revision: number; request_id: string; request_payload: FeeRequest; amount_micro_usd: number; evidence_reference: string; note: string; source: 'owner_declared'; created_at: string };
