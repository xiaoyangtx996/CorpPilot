...
export type CliSettingsValue = { resource_admission_enabled: boolean; host_reserve_memory_mb: number; local_worker_memory_mb: number; local_worker_cpus: number; enabled: boolean; backend: 'local' | 'docker'; executable: string; docker_executable: string; docker_image: string; docker_cpus: number; docker_memory_mb: number; docker_pids_limit: number; model: string; api_key_env: string; timeout_seconds: number; max_concurrency: number; configured: boolean; credential_available: boolean; executable_available: boolean; platform_supported: boolean };
