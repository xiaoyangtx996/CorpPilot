...
export function TaskDependencies({ task, onClose, onUpdate }: { task: Task; onClose: () => void; onUpdate: (task: Task) => void }) {
  ...
  return <dialog ref={dialog} className="task-dialog" aria-labelledby="dependencies-title" onCancel={event => { event.preventDefault(); if (!busy) onClose(); }}>
    ...
    {current && <section aria-label="当前前置状态"><h3>{checked ? '当前需求' : '上次读取需求'} v{current.status.requirement_version}</h3>{!checked && <p role="status">以下为上次读取状态，可能已过期；尚未完成最新核对。</p>}<p>{current.status.ready ? current.status.task_ids.length ? '前置成果已满足批准条件。' : '未设置前置任务。' : `前置条件未满足：${current.status.blocked_reason}`}</p><p className="muted">以上仅表示前置条件，不表示本任务已执行或已验收。执行时仍会重新核查。</p><ul>{current.status.task_ids.map(id => <li key={id}>{title(id)}</li>)}</ul>{!current.editable && <p className="archive-notice">当前为只读：会话已归档，或负责人未启用 / 不在会话中。</p>}</section>}
