...
import { api, ApiError, type ModelReconciliationRecord, type Agent, type Conversation, type Message, type PlanningRequest, type PlanningRun, type CollaborationPlan, type ModelSettingsValue, type ReplyRuntime } from './api';
...
export function PlanningPanel({ conversationId, source, onClose, onImport, onRecoverCollaboration }: { conversationId: string; source?: Message; onClose: () => void; onImport: (conversationId: string, plan: CollaborationPlan) => void; onRecoverCollaboration: () => void }) {
  ...
  async function release(rejected = false) {
    if (writing.current || storageError || (!rejected && (!current || !(['completed', 'failed', 'cancelled'].includes(current.state) || current.state === 'unknown' && !!checkedRuns[current.id]))) || rejected && !pendingRef.current?.rejected) return;
