...
import { GoalExecutionPanel } from './GoalExecutionPanel';
import { MemoryPanel } from './MemoryPanel';

function Icon({ name }: { name: 'workspace' | 'chat' | 'group' | 'plus' | 'search' | 'settings' | 'activity' | 'person' | 'panel' }) {
  const paths = {
    workspace: 'M4 5h16v14H4zM9 5v14M9 10h11',
    chat: 'M20 11a8 8 0 0 1-8 8H5l-3 3v-8a8 8 0 1 1 18-3Z',
    group: 'M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2M16 3a4 4 0 0 1 0 8M22 21v-2a4 4 0 0 0-3-3.87M13 7a4 4 0 1 1-8 0 4 4 0 0 1 8 0Z',
...
export default function App() {
  const [goalExecution, setGoalExecution] = useState<{ conversationId: string; source?: Message } | null>(null);
  ...
  const [peerReview, setPeerReview] = useState<{ conversationId: string; source?: Message } | null>(null);
  ...
  const [budgetPanel, setBudgetPanel] = useState<{ agent?: Agent } | null>(null);
  const [repositoryProject, setRepositoryProject] = useState<string | null>(null);
  ...
  return <div className="app" aria-busy={loading}>
    <aside ref={navigationRef} className={`navigation ${navigation ? 'open' : ''}`} aria-label="会话与 Agent" role={navigation ? 'dialog' : undefined} aria-modal={navigation || undefined} inert={inspector}>
      <div className="brand"><span className="brand-mark"><Icon name="workspace" /></span><span>CorpPilot<small>个人 AI 工作空间</small></span><button className="mobile" onClick={() => setNavigation(false)}>关闭</button></div>
      <section className="conversations"><header><h2>会话</h2><button className="text-button" disabled={!agents.some(agent => agent.enabled)} onClick={() => setConversationEditor(null)}>新建群聊</button></header>
        <label className="check archive-filter"><input type="checkbox" checked={showArchived} onChange={event => setShowArchived(event.target.checked)} />显示已归档</label>
        <div className="conversation-list">{conversations.filter(item => showArchived || !item.archived).map(item => <button key={item.id} aria-current={item.id === conversationId ? 'page' : undefined} className={`conversation-item ${item.id === conversationId ? 'selected' : ''}`} onClick={() => chooseConversation(item)}><span className="conversation-icon"><Icon name={item.type === 'dm' ? 'chat' : 'group'} /></span><span className="conversation-copy"><strong>{item.title}</strong><small>{item.archived ? '已归档' : item.type === 'dm' ? '私聊' : item.type === 'board' ? '董事会' : '项目群'} · {item.last_message?.content ?? '暂无消息'}</small></span></button>)}{!conversations.some(item => showArchived || !item.archived) && <p className="muted">暂无会话，选择 Agent 开始私聊。</p>}</div>
      </section>
      <section className="agent-list">{agentSaveNotice && <p role="status">{agentSaveNotice}</p>}<header><h2>Agent <span className="count">{agents.length}</span></h2><button className="text-button" onClick={() => setEditor(null)}>新建</button></header>
        <label className="search"><Icon name="search" /><span className="sr-only">搜索 Agent</span><input placeholder="搜索 Agent" value={query} onChange={e => setQuery(e.target.value)} /></label>
        <div className="people">{agents.filter(person => person.name.toLowerCase().includes(query.toLowerCase())).map(person => <button key={person.id} className={`person ${person.id === selected ? 'selected' : ''}`} onClick={() => void openDm(person)}>
          <span className={`avatar ${person.enabled ? '' : 'disabled-avatar'}`}>{person.name.charAt(0)}</span><span><strong>{person.name}</strong><small><span className={`status-dot ${person.enabled ? '' : 'disabled-dot'}`} aria-hidden="true" />{person.enabled ? '可用' : '已停用'}</small></span>
      ...
      <div className="navigation-settings"><details className="management-tools"><summary><Icon name="activity" />管理工具</summary><div className="management-actions"><button onClick={() => { setNavigation(false); setGoalExecution({ conversationId }); }}>目标执行与恢复</button><button onClick={() => { setNavigation(false); setPeerReview({ conversationId }); }}>Agent评议与恢复</button><button onClick={() => { setNavigation(false); setModelReconciliation(true); }}>模型调用核查</button><button onClick={() => { setNavigation(false); setPlanning({ conversationId }); }}>模型协作提案</button><button onClick={() => { setNavigation(false); setCollaboration({ conversationId }); }}>协作计划与恢复</button><button onClick={() => { setNavigation(false); setReconciliation(true); }}>执行核查</button><button onClick={() => { setNavigation(false); setBudgetPanel({}); }}>预算与预留</button></div></details><div className="settings-shortcuts"><button onClick={() => setModelSettings(true)}><Icon name="settings" />模型设置</button><button onClick={() => setCliSettings(true)}><Icon name="workspace" />CLI 设置</button></div><div className="owner-caption"><span className="owner-monogram">O</span><span>Owner<small>你的工作空间，你来掌舵</small></span></div></div>
    </aside>
    <main className="workspace" inert={navigation || inspector}>
      <header className="topbar"><button className="mobile" aria-expanded={navigation} onClick={() => setNavigation(true)}><Icon name="panel" />会话与 Agent</button><div className="workspace-heading"><span className="eyebrow">工作空间</span><h1>{conversation?.title ?? agent?.name ?? 'Agent 工作台'}</h1></div><span className="workspace-mode"><Icon name="chat" />协作会话</span><button className="inspector-toggle" aria-expanded={inspector} onClick={() => setInspector(true)}><Icon name="panel" />Agent 视角</button></header>
      ...
      {conversation ? <><div className="conversation-toolbar"><span className="muted">{conversation.type === 'dm' ? '私聊' : conversation.type === 'board' ? '董事会' : '项目群'} · {conversation.member_ids.length} 位 Agent{conversation.archived ? ' · 已归档' : ''}</span><button onClick={() => setConversationEditor(conversation)}>管理会话</button>{conversation.type !== 'dm' && <button onClick={() => setMemory({ scope: 'project', identity: conversation.id, title: conversation.title, conversationId: conversation.id })}>项目共享记忆</button>}{conversation.type === 'project' && <button onClick={() => setRepositoryProject(conversation.id)}>项目代码仓库</button>}<button disabled={conversationBusy} onClick={() => void archive()}>{conversation.archived ? '恢复会话' : '归档会话'}</button></div><ConversationMessages key={conversation.id} conversation={conversation} agents={agents} draft={drafts.current[conversation.id]} onMessage={receivedMessage} onGoalExecution={source => setGoalExecution({ conversationId: conversation.id, source })} onPeerReview={source => setPeerReview({ conversationId: conversation.id, source })} onPlanning={source => setPlanning({ conversationId: conversation.id, source })} onCollaboration={source => setCollaboration({ conversationId: conversation.id, source })} onSettings={() => setModelSettings(true)} /></> : <div className="empty-workspace"><div className="large-avatar">{agent?.name.charAt(0) ?? 'C'}</div><h2>{loading ? '正在读取工作台…' : '选择会话开始讨论'}</h2><p>选择左侧 Agent 开启私聊，或创建董事会与项目群。消息会持久保存；选定成员并确认后，才会调用已配置模型回复。</p>{agent && <button onClick={() => setEditor(agent)}>编辑身份</button>}</div>}
    </main>
    <aside ref={inspectorRef} className={`inspector ${inspector ? 'open' : ''}`} aria-label="Agent 视角" role={inspector ? 'dialog' : undefined} aria-modal={inspector || undefined} inert={navigation}><header><h2><Icon name="person" />Agent 视角</h2><span className="inspector-caption">观察与管理</span><button className="inspector-toggle" onClick={() => setInspector(false)}>关闭</button></header>
      ...
      {agent ? <><section><div className="identity-heading"><span className="avatar">{agent.name.charAt(0)}</span><div><h2>{agent.name}</h2><small>{agent.enabled ? '可用' : '已停用'}</small></div></div><p className="muted">身份可用不代表执行实例正在运行。</p></section>
        ...
        <section><h3>持久记忆</h3><button onClick={() => { setInspector(false); setMemory({ scope: 'agent', identity: agent.id, title: agent.name, conversationId: conversationId }); }}>个人记忆</button></section><section><button onClick={() => { setInspector(false); setBudgetPanel({ agent }); }}>查看预算预留</button></section><AgentActivity key={agent.id} agent={agent} agents={agents} onOpenConversation={row => { changedConversation(row); chooseConversation(row); setInspector(false); }} />
    ...
    {repositoryProject && <RepositorySettings key={repositoryProject} conversationId={repositoryProject} onClose={() => setRepositoryProject(null)} />}
    {peerReview && <PeerReviews {...peerReview} onClose={() => setPeerReview(null)} />}
    {goalExecution && <GoalExecutionPanel {...goalExecution} onClose={() => setGoalExecution(null)} onOpenProject={project => { setConversations(rows => [project, ...rows.filter(row => row.id !== project.id)]); chooseConversation(project); }} />}
    ...
    {budgetPanel && <BudgetPanel key={budgetPanel.agent?.id ?? "global"} agent={budgetPanel.agent} onClose={() => setBudgetPanel(null)} />}{cliSettings && <CliSettings onClose={() => setCliSettings(false)} />}
