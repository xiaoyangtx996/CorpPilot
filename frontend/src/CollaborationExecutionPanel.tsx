...
export function CollaborationExecutionPanel({ plan, onClose }: { plan: CollaborationReceipt; onClose: () => void }) {
  const dialog = useRef<HTMLDialogElement>(null), alive = useRef(false), serial = useRef(0), reading = useRef(false), writing = useRef(false), pendingRef = useRef<Pending | null>(null), viewRef = useRef('');
  ...
  const [selected, setSelected] = useState<string[]>([]), [notes, setNotes] = useState<Record<string, string>>({}), [confirmed, setConfirmed] = useState(false), [stopConfirmed, setStopConfirmed] = useState(false), [stopUnknown, setStopUnknown] = useState(false);
  ...
  useEffect(() => {
    ...
    try { const raw = sessionStorage.getItem(key), saved = raw ? decode(raw, Object.values(plan.task_ids)) : null; pendingRef.current = saved; setPending(saved); }
    catch { setStorageError('无法读取原批量请求，请恢复会话存储后重新打开；不能新建请求。'); }
  ...
  function release() {
    ...
    try { sessionStorage.removeItem(key); pendingRef.current = null; setPending(null); setDetail(null); viewRef.current = ''; setSelected([]); setNotes({}); setConfirmed(false); setStopConfirmed(false); setStopUnknown(false); serial.current++; void load(); }
    catch { setStorageError('无法结束原请求查看，继续保持锁定。'); }
  }
  async function stop() {
    if (writing.current || !detail || !stopConfirmed || stopUnknown) return;
    const id = detail.id;
    try { sessionStorage.setItem(`corppilot.project-stop-pending.v1.${id}`, 'unconfirmed'); } catch { setStorageError('无法保存停止核对记录，尚未发送。'); return; }
    writing.current = true; serial.current++; setBusy(true); setStopUnknown(true); setStopConfirmed(false); setError('');
  ...
  return <dialog ref={dialog} className="task-dialog" aria-labelledby="batch-title" onCancel={event => { event.preventDefault(); if (!busy) onClose(); }}><header><h2 id="batch-title">批量执行协作项目 · {plan.approved_plan.title}</h2><button disabled={busy} onClick={onClose}>关闭</button></header>
    <p>只选择本协作回执中的 1–16 个任务，按各自当前需求版本一起入队。确认可能调用 CLI 和模型并产生费用；批量创建全有或全无。前置成果仍须 Owner 批准，未选前置任务不会自动执行。</p><small>协作回执 {plan.id}</small><button disabled={busy || loading} onClick={() => void load()}>刷新任务与本批状态</button>
    ...
    {detail && <section><h3>本批绑定实例</h3><small>批次 {detail.id}</small>{detail.items.map(item => <article className="task-card" key={item.execution.id}><h4>{item.task.title} · {labels[item.execution.state]}</h4><small>执行 {item.execution.id} · v{item.execution.requirement_version}</small>{item.latest_execution_id !== item.execution.id && <p>此实例不是任务最新执行；本批停止不会操作替代实例。</p>}{item.task.requirement_version !== item.execution.requirement_version && <p>当前需求已变化；本批仍绑定原需求版本。</p>}{item.execution.state === 'unknown' && <p>{declarations[item.execution.id] ? 'Owner 已声明进程停止并核查外部影响；原执行仍为未知。再次执行仍须填写说明并重新确认。' : '未知实例尚未核查或声明未读到，请到任务执行中核查后刷新本页。'}</p>}<p>Owner 评审：{item.review?.decision === 'approved' ? '已批准' : item.review?.decision === 'rejected' ? '已拒绝' : '尚未批准'}</p>{!item.dependencies.ready && <p>等待前置：{item.dependencies.blocked_reason}</p>}{item.execution.summary && <p>{item.execution.summary}</p>}</article>)}<p>执行结束不等于成果验收通过；请在项目任务中评审成果或核查未知执行。</p>
      {stopUnknown ? <p className="error">停止响应未确认。只刷新本批状态，不自动重发停止；必要时到具体任务核查原实例。</p> : detail.items.some(i => active(i.execution)) && <><label className="check"><input type="checkbox" disabled={busy || loading} checked={stopConfirmed} onChange={event => setStopConfirmed(event.target.checked)} />我确认只请求停止本批绑定的实例，等待实际退出核实</label><button disabled={busy || loading || !stopConfirmed} onClick={() => void stop()}>确认停止本批执行</button></>}
    </section>}
    <details><summary>批量执行历史</summary>{history.map(row => <p key={row.id}><button disabled={busy || loading || !!pending} onClick={() => { viewRef.current = row.id; setDetail(null); setStopUnknown(false); void load(); }}>查看批次 {row.id}</button> · {row.created_at}</p>)}</details>
  </dialog>;
}
