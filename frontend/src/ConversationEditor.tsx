...
export function ConversationMessages({ conversation, agents, draft, onMessage, onSettings, onCollaboration }: {
  conversation: Conversation; agents: Agent[]; draft: Draft; onMessage: (message: Message) => void; onSettings: () => void; onCollaboration: (source: Message) => void;
}) {
  ...
  return <>
    <div className="message-history" aria-label="消息历史" aria-busy={loading}>
      ...
      {messages.map(message => <article className={`message ${message.sender_kind}`} key={message.id} id={`message-${message.id}`}><header><strong>{message.sender_kind === 'owner' ? '你 · Owner' : agents.find(agent => agent.id === message.sender_id)?.name ?? 'Agent'}</strong><time dateTime={message.created_at}>{new Date(message.created_at).toLocaleString()}</time></header><p>{message.content}</p>{message.sender_kind === 'owner' && <><button type="button" className="text-button" disabled={conversation.archived} onClick={() => setReplySource(message)}>请 Agent 回复</button><button type="button" className="text-button" disabled={conversation.archived} onClick={() => setTaskSource({ message })}>创建任务</button><button type="button" className="text-button" disabled={conversation.archived} onClick={() => onCollaboration(message)}>组建协作项目</button></>}</article>)}
