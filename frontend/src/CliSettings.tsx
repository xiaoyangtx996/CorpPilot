    ...
    <form onSubmit={save}><header><h2 id="cli-settings-title">CLI 设置</h2><button type="button" disabled={busy !== null} onClick={onClose}>关闭</button></header>
      ...
      {saved && draft && <>
        ...
        <fieldset className="settings-fields" disabled={busy !== null || loading}><legend className="sr-only">CLI 配置字段</legend>
          ...
          {draft.engine === 'opencode' && <p>OpenCode Zen 可使用本地 CLI 或 Docker Worker，只开放原生文件工具，禁用 Shell、网络工具和子代理。{draft.backend === 'local' && '本地使用独立工作目录与配置；目录隔离不是操作系统沙箱。'}切换工具或后端不会自动更改路径、镜像、模型或密钥环境变量。</p>}
          {draft.engine === 'opencode' && draft.backend === 'docker' && <p>OpenCode Docker Worker 需要专用固定 Linux 镜像，包含 OpenCode 1.18.29，并声明镜像能力标签 io.corppilot.opencode=1.18.29。</p>}
        ...
        <p className="muted">{saved.backend === 'docker' ? `检查只读取本地 Docker 服务和固定 Linux 镜像状态${saved.engine === 'opencode' ? '，并核对 OpenCode 镜像能力标签' : ''}，不启动容器、不拉取镜像。` : '检查只运行已保存程序的 --version。'}不调用模型、不验证模型账号。{dirty ? '有未保存修改，请先保存，再手动检查。' : '检查成功不代表任务已调度或执行。'}</p>
      </>}
