...
export function CollaborationPanel({ conversationId, source, initialPlan, onClose, onOpenProject }: { conversationId: string; source?: Message; initialPlan?: CollaborationPlan; onClose: () => void; onOpenProject: (conversation: Conversation) => void }) {
  ...
  const cycle = cyclic(plan.tasks), oversized = new TextEncoder().encode(JSON.stringify(operation === 'launch' ? { plan, confirm_execution: true } : plan)).length > 65536;
  ...
  async function submitLaunch() {
    ...
    const sent: LaunchPending = saved ? { source_id: saved.source_id, operation: 'launch', payload: saved.payload } : { source_id: sourceId, operation: 'launch', payload: { plan: structuredClone(plan), confirm_execution: true } };
  ...
  function finishLaunch() {
    ...
    try { sessionStorage.removeItem(launchKey); launchRef.current = null; setLaunchPending(null); setReceipt(launchReceipt.collaboration); setConfirmed(false); setNotice('已独立读回项目和固定首批回执，任务已获执行授权；入队不等于执行成功，成果仍需Owner审批。'); }
  ...
  return <dialog ref={dialog} className="task-dialog" aria-labelledby="collaboration-title" onCancel={event => { event.preventDefault(); if (!busy) onClose(); }}><header><h2 id="collaboration-title">组建协作项目</h2><button disabled={busy} onClick={onClose}>关闭</button></header>
    ...
    <p>从 Owner 消息组织项目群与任务。仅共享你明确填写的摘要；源会话历史不会自动复制。仅创建项目不启动执行；选择批准并启动时，将授权全部任务进入CLI队列，可能调用模型并产生费用。前置成果仍须Owner批准。</p>
    ...
    {launchPending ? <section><h3>原创建并启动请求待核对</h3><small>源会话 {launchPending.source_id} · 请求 {launchPending.payload.plan.request_id} · 操作：批准并启动全部任务</small>{summary(launchPending.payload.plan)}<p>完整计划已冻结。若尚未受理，同键核对可能首次创建项目并启动已授权的全部任务；可能产生CLI和模型费用。</p><button disabled={busy || loading || !!storageError} onClick={() => void submitLaunch()}>{launchPending.launch_id ? '读取原启动回执（不再启动）' : '同键核对原启动请求'}</button>{launchPending.rejected && <button disabled={busy || !!storageError} onClick={editLaunchRejected}>修改首次未接受的启动计划</button>}{launchReceipt ? <><p>已独立GET核对项目 {launchReceipt.collaboration.project_conversation_id} 和固定批次 {launchReceipt.batch.id}，包含 {launchReceipt.batch.tasks.length} 个首次执行；入队不等于成功。</p><button disabled={busy || loading || !!readError || !!storageError} onClick={finishLaunch}>确认已读回并结束原请求核对</button><button disabled={busy || !!storageError} onClick={() => showBatch(launchReceipt.collaboration, launchReceipt.batch.id)}>查看已启动批次与停止控制</button></> : <p>尚未独立读回完整组合回执，原请求保留，不能新建替代项目。</p>}</section>
      ...
      : plan.source_message_id ? <form onSubmit={event => { event.preventDefault(); void submit(); }}><small>原消息 ID：{plan.source_message_id}</small>{source?.id === plan.source_message_id && <details><summary>查看原 Owner 消息（不会自动共享）</summary><p className="task-source">{source.content}</p></details>}<fieldset className="task-fields" disabled={busy || loading || !!readError || !!storageError || !conversation || conversation.archived}>
        ...
        <section><h3>提交前确认</h3><label>本次操作<select value={operation} onChange={event => { setOperation(event.target.value as 'create' | 'launch'); setConfirmed(false); }}><option value="create">仅创建项目（不启动执行）</option><option value="launch">批准完整计划并启动全部任务</option></select></label><p>将加入项目群：{members.map(id => agents.find(a => a.id === id)?.name ?? id).join('、') || '尚未选择'}</p>{summary(plan)}<label className="check"><input type="checkbox" checked={confirmed} onChange={event => setConfirmed(event.target.checked)} />{operation === 'launch' ? '我批准上述完整计划、共享摘要、全部任务范围与验收及依赖，并授权创建项目后将全部任务首次入队；可能调用CLI和模型并产生费用，前置成果仍须Owner批准' : '我确认向上述成员共享摘要，并按此计划创建项目群、任务及依赖'}</label></section><button className="primary" disabled={invalid || !confirmed}>{operation === 'launch' ? '批准并启动全部任务' : '确认创建协作项目'}</button>
    ...
    <section><h3>创建并启动历史</h3>{loaded && !loading && !readError && !launchHistory.length && <p>暂无创建并启动记录。</p>}{launchHistory.map(row => <details key={row.id}><summary>{row.collaboration.approved_plan.title} · {row.created_at}</summary>{summary(row.request_payload.plan)}<p>启动回执 {row.id} · 固定首批 {row.batch.id}；入队不等于完成。打开控制页将定位此编号，已有原批量请求则优先恢复。</p><button disabled={busy} onClick={() => void openProject(row.collaboration)}>打开此项目群</button><button disabled={busy} onClick={() => showBatch(row.collaboration, row.batch.id)}>查看已启动批次与停止控制</button></details>)}</section>
